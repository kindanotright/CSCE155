...
int main(int argc, char **argv) {
  ...
  FILE* mlbTempData = fopen("mlb_nl_2011.xml", "r");
  if (mlbTempData == NULL) {
      return 1;
  }
  char mlbWinLossCopy[maxSize];
  double wins = 0;
  double losses = 0;
  for (i=0; i < numTeams; i++) {
    if (fgets(mlbWinLossCopy, sizeof(mlbWinLossCopy), mlbTempData) != NULL) {
      const char delim[2]=" ";
      char *token = strtok(mlbWinLossCopy, delim);
      if (token) {
        strncpy(teams[i], token, maxSize);
        teams[i][maxSize - 1] = '\0';
      }
      wins = strtod(strtok(NULL, delim), NULL);
      losses = strtod(strtok(NULL, delim), NULL);
      winPercentages[i]=(double)wins / (double)(wins+losses);
    }
  }
  fclose(mlbTempData);
